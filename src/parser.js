...
function extractWikilinks(body) {
  // Negative lookbehind for ! to skip Obsidian image embeds ![[...]]
  const re = /(?<!!)\[\[([^\]|#]+)(?:#[^\]|]*)?(?:\|[^\]]*)?\]\]/g;
  const links = [];
  let m;
  while ((m = re.exec(body)) !== null) {
    ...
    if (name && !/\.(png|jpg|jpeg|gif|webp|svg|heic|pdf)$/i.test(name)) links.push(name);
  }
  return links;
}
...
// Directories to always skip when scanning for notes
const SKIP_DIRS = new Set(['node_modules', 'site', 'src', 'public', 'scripts', '.git', '.obsidian', '.vscode', '.github']);
